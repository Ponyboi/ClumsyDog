...
//			Debug.Log (verticalSpeed);
	...
	public void grabObject() {
		...
		if (ControllerInput.X_Button(id) || (allowKeyboardInput && Input.GetMouseButton(1))) {
			grabSphere.GetComponent<Renderer>().enabled = true;
			...
			grabSphere.GetComponent<Renderer>().enabled = false;
	...
	public void throwObject() {
		if (ControllerInput.B_ButtonDown(id) || (allowKeyboardInput && Input.GetMouseButtonDown(0))) {
			...
			if (grabbedObject != null) {
				...
				throwDir += Vector3.up;
				grabbedObject.GetComponent<Rigidbody>().AddForce(throwDir * throwStrength);
				grabbedObject = null;
			}
		}
	}
